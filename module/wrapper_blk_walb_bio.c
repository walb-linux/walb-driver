...
/* debug */
static atomic_t n_wq_logpack_ = ATOMIC_INIT(0);
...
/* pdata->flags bit. */
enum {
	PDATA_STATE_READ_ONLY = 0,
	PDATA_STATE_FAILURE,
	PDATA_STATE_QUEUE_STOPPED,
...
static struct pack_work* enqueue_task_if_necessary(
	struct wrapper_blk_dev *wdev,
	int nr, struct workqueue_struct *wq, int cpu,
	void (*task)(struct work_struct *));
#endif
...
static bool create_private_data(struct wrapper_blk_dev *wdev)
{
	...
	spin_lock_init(&pdata->datapack_submit_queue_lock);
...
static void stop_dev(void)
{
	...
	LOGn("wq(logpack %d io %d ol %d unbound %d)\n",
		atomic_read(&n_wq_logpack_),
		atomic_read(&n_wq_io_),
		atomic_read(&n_wq_ol_),
		atomic_read(&n_wq_unbound_)); /* debug */
	...
	struct wrapper_blk_dev *wdev, struct pack *wpack)
{
	...
	list_for_each_entry_safe(biow, biow_next, &wpack->biow_list, list) {
		...
		retry_insert_pending:
			...
			if (is_stop_queue) {
				ret = test_and_set_bit(PDATA_STATE_QUEUE_STOPPED,
						&pdata->flags);
				ASSERT(!ret);
			}
			...
			retry_pack_work:
				...
				queue_delayed_work(system_wq, &pwork->dwork,
		...
		retry_insert_ol:
			...
			atomic_inc(&biow->submitter); /* debug */
		}
		continue;
	error_io:
		is_failed = true;
...
static void gc_logpack_list(struct pdata *pdata, struct list_head *wpack_list)
{
	...
	list_for_each_entry_safe(wpack, wpack_next, wpack_list, list) {
		...
		retry:
			...
			if (rtimeo == 0) {
				...
				LOGn("submitter %d waiter %d wq(logpack %d io %d ol %d unbound %d) n_pending_bio %d\n",
					atomic_read(&biow->submitter), atomic_read(&biow->waiter),
					atomic_read(&n_wq_logpack_),
					atomic_read(&n_wq_io_),
					atomic_read(&n_wq_ol_),
					atomic_read(&n_wq_unbound_),
					atomic_read(&n_pending_bio_)); /* debug */
				LOGn("lsubmit %u lwait %u dsubmit %u gc %u\n", pdata->logpack_submit_queue_n,
					pdata->logpack_wait_queue_n,
					pdata->datapack_submit_queue_n,
					pdata->logpack_gc_queue_n); /* debug */
	...
	pdata->written_lsid = written_lsid;
	...
	struct wrapper_blk_dev *wdev, struct bio_wrapper *biow)
{
	...
	if (is_start_queue) {
		ret = test_and_clear_bit(PDATA_STATE_QUEUE_STOPPED,
					&pdata->flags);
		ASSERT(ret);
	}
	pdata->pending_sectors -= biow->len;
...
static void task_submit_bio_wrapper_list(struct work_struct *work)
{
	...
	while (true) {
		...
		/* Dequeue all bio wrappers from the submit queue. */
		spin_lock(&pdata->datapack_submit_queue_lock);
		is_empty = list_empty(&pdata->datapack_submit_queue);
		if (is_empty) {
...
static void wrapper_blk_make_request_fn(struct request_queue *q, struct bio *bio)
{
	...
	if (biow->bio->bi_rw & REQ_WRITE) {
		...
		if (!test_bit(PDATA_STATE_QUEUE_STOPPED, &pdata->flags)) {
			enqueue_submit_task_if_necessary(wdev);
		}
	} else { /* read */
		submit_read_bio_wrapper(wdev, biow);

		/* TODO: support PDATA_STATE_QUEUE_STOPPED for read also. */
	}
	return;
#if 0
error1:
 ...
 * @nr flag bit number.
 * @wq workqueue.
 * @cpu cpu id. negative number means no choice.
 * @task task.
 *
 * RETURN:
...
retry:
	if (!test_and_set_bit(nr, &pdata->flags)) {
		...
		if (cpu < 0) {
			...
			ret = queue_work_on(cpu, wq, &pwork->work);
		}
		if (!ret) {
...
{
#if 1
	if (enqueue_task_if_necessary(wdev,
					PDATA_STATE_SUBMIT_TASK_WORKING,
					wq_logpack_, -1,
...
static void enqueue_gc_task_if_necessary(struct wrapper_blk_dev *wdev)
{
	...
	pwork = enqueue_delayed_task_if_necessary(
		wdev,
		PDATA_STATE_GC_TASK_WORKING,
		wq_logpack_,
